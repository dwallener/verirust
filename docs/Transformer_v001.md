...
## Overview
This document defines the canonical transformer contract used by the `verirust` project.

The goal is to ensure a Rust reference implementation (Phase 1) and a Verilog hardware
implementation (Phase 2) can execute **identical transformer inference**.
...
Rust serves as the **semantic reference implementation**.
Verilog must match the **canonical fixed‑point Rust outputs bit‑for‑bit**.
...
Rust may compute exact exp in reference mode.
Canonical mode must use shared LUT approximations.
...
Both implementations must expose intermediate tensors:
...
Canonical mode comparison requires:
...
- implements full forward pass
- supports float reference mode
- supports Q8.8 canonical mode
- loads canonical weight blob
- emits checkpoint dumps
- deterministic execution

Rust fixed‑point mode defines the **hardware reference behavior**.

---

# Phase 2: Verilog Implementation

Requirements:

- identical tensor semantics
- identical numeric behavior
- mutable weight memory
...
Weights must **not** be baked into logic yet.
...
- Verilog simulation matches Rust canonical mode
- all checkpoint tensors match bit‑for‑bit
- synthesizes successfully
- timing and area reports generated

---

# Phase 3 (Future)

Once Phase 1 and 2 are verified:

- freeze weights
- synthesize optimized hardware
- compare area, latency, and throughput
