...
module verirust_flexible_synth (
    clk,
    rst_n,
    start,
    cfg_we,
...
wire ctrl_done;
reg [7:0] token_rd_addr;
wire [7:0] token_rd_data;
reg [3:0] rd0_space;
reg [15:0] rd0_addr;
wire signed [15:0] rd0_data;
reg [3:0] rd1_space;
reg [15:0] rd1_addr;
wire signed [15:0] rd1_data;
...
verirust_stage_embed_tok stage_embed_tok (
    ...
    .token_value(token_rd_data),
    .tok_embed_value(rd0_data),
...
verirust_stage_softmax stage_softmax (
    ...
    .exp_lut_value(rd0_data),
    .recip_lut_value(rd0_data),
...
always @* begin
    token_rd_addr = 8'd0;
    rd0_space = CFG_TOK_EMBED;
    rd0_addr = 16'd0;
    rd1_space = CFG_POS_EMBED;
    rd1_addr = 16'd0;

    if (en_embed_tok) begin
        token_rd_addr = embed_tok_token_rd_addr;
        rd0_space = CFG_TOK_EMBED;
        rd0_addr = embed_tok_tok_embed_addr;
    end else if (en_embed_add) begin
        rd1_space = CFG_POS_EMBED;
        rd1_addr = embed_add_pos_addr;
    end else if (en_norm1) begin
        if (norm1_use_rsqrt) begin
            rd0_space = CFG_RSQRT_LUT;
            rd0_addr = norm1_rsqrt_addr;
        end else if (norm1_use_weight) begin
            rd0_space = CFG_NORM1;
            rd0_addr = norm1_weight_addr;
        end
    end else if (ctrl_phase == PH_Q) begin
        rd0_space = CFG_W_Q;
        rd0_addr = q_weight_addr;
    end else if (ctrl_phase == PH_K) begin
        rd0_space = CFG_W_K;
        rd0_addr = k_weight_addr;
    end else if (ctrl_phase == PH_V) begin
        rd0_space = CFG_W_V;
        rd0_addr = v_weight_addr;
    end else if (en_softmax) begin
        if (softmax_use_exp_lut) begin
            rd0_space = CFG_EXP_LUT;
            rd0_addr = softmax_exp_lut_addr;
        end else if (softmax_use_recip_lut) begin
            rd0_space = CFG_RECIP_LUT;
            rd0_addr = softmax_recip_lut_addr;
        end
    end else if (en_norm2) begin
        if (norm2_use_rsqrt) begin
            rd0_space = CFG_RSQRT_LUT;
            rd0_addr = norm2_rsqrt_addr;
        end else if (norm2_use_weight) begin
            rd0_space = CFG_NORM2;
            rd0_addr = norm2_weight_addr;
        end
    end else if (en_ffn1) begin
        rd0_space = CFG_W1;
        rd0_addr = ffn1_weight_addr;
    end else if (en_ffn1_relu) begin
        rd0_space = CFG_TOK_EMBED;
        rd0_addr = 16'd0;
    end else if (ctrl_phase == PH_ATTN_OUT) begin
        rd0_space = CFG_W_O;
        rd0_addr = attn_out_weight_addr;
    end else if (en_ffn2) begin
        rd0_space = CFG_W2;
        rd0_addr = ffn2_weight_addr;
    end else if (en_logits) begin
        rd0_space = CFG_LM_HEAD;
        rd0_addr = logits_weight_addr;
    end
end
